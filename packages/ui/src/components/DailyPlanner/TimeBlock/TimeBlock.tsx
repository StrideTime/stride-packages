...
export function TimeBlock({
  ...
  const { resizing, displayTop, displayHeight, handleResizeStart, justResizedRef } =
    useTimeBlockResize({
      top,
      height,
  ...
  if (isExternalEvent && event.metadata) {
    try {
      externalCalendarName = JSON.parse(event.metadata).calendarName;
    } catch (e) {
      console.error(e);
    }
  }
  ...
  return (
    <div
      ...
      className={cn(
        styles.block,
        colorClass,
        isExternalEvent && styles.blockExternal,
      ...
      <div className={styles.body}>
        {isTinyEvent ? (
          <div className={styles.tinyRow}>
            ...
            <span className={styles.titleTiny}>{task ? task.title : event.label}</span>
            <span className={styles.startTime}>{formatTime12Hour(event.startTime)}</span>
            {!isExternalEvent && onRemove && (
              <Button
                ...
                className={cn(
                  styles.removeBtn,
                  styles.removeBtnTiny,
                  isHovered ? styles.btnVisible : styles.btnHidden
                )}
          ...
          <>
            <div className={styles.smallHeader}>
              ...
              <span className={styles.titleSmall}>{task ? task.title : event.label}</span>
              {!isExternalEvent && onRemove && (
                <Button
                  size="icon"
                  variant="ghost"
          ...
          <>
            <div className={styles.largeHeader}>
              ...
              <span className={styles.largeTitle}>{task ? task.title : event.label}</span>
              {onRemove && (
                <Button
                  size="icon"
                  variant="ghost"
